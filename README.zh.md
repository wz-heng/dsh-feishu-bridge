...
- 用这个 bridge 的进程时用composition 实际需要的最小权限运行。内置默认的 `dsh` composition（`examples/jsonrpc-agent` 上游）用的是 `danger-full-access` 的 bash——请在一次性工作区/容器里跑，不要对着你在意的机器跑，不论是否同时开启审批模式（这是两个互相独立的控制手段，见"远程工具审批"）。
